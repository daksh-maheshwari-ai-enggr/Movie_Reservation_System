...
export function AdminPanel() {
  ...
  if (activeTab === 'movies') {
    activeContent = <MoviesTable onAddFilm={() => setIsFilmModalOpen(true)} />
  }

  if (activeTab === 'theaters') {
    activeContent = <TheatersTable onAddTheater={() => setIsTheaterModalOpen(true)} />
  }

  if (activeTab === 'showtimes') {
    activeContent = <ShowtimesTable onAddShowtime={() => setIsShowtimeModalOpen(true)} />
  }
    ...
    <div className="min-h-screen bg-[#08080f] text-[#f7f3ed] selection:bg-[#d69b22] selection:text-black">
      ...
      <main className="mx-auto w-full max-w-[1920px] px-5 py-10 sm:px-9 lg:px-12">
        ...
        <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard label="Theaters" value={theaters.length} />
          <StatCard label="Confirmed bookings" value={bookings.length} />
          <StatCard label="Revenue" value="$33.60" />
        </section>

        <AdminTabs activeTab={activeTab} onTabChange={setActiveTab} />

        <section className="pt-10">{activeContent}</section>
      </main>
      ...
      {isFilmModalOpen && <AddFilmModal onClose={() => setIsFilmModalOpen(false)} onFilmAdded={handleFilmAdded} />}
      {isTheaterModalOpen && <AddTheaterModal onClose={() => setIsTheaterModalOpen(false)} onTheaterAdded={handleTheaterAdded} />}
      {isShowtimeModalOpen && <AddShowtimeModal onClose={() => setIsShowtimeModalOpen(false)} onShowtimeAdded={handleShowtimeAdded} />}
    </div>
  )
}
