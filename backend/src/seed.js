...
await Promise.all(
  [User, Movie, Theater, Seat, Showtime, ShowtimeSeat, Booking, Payment].map(
    (M) => M.deleteMany({}),
  ),
);
...
const movies = await Movie.insertMany(
  films.map(([title, genre, duration, rating, posterUrl], i) => ({
    title,
    genre,
    duration,
    rating,
    year: 2026,
    director: ["Avery Ross", "Marco Albanese", "Elena Finch"][i % 3],
    cast: ["Simone Delacroix", "Ravi Patel", "Kira Weston"],
    description: `A captivating ${genre.toLowerCase()} story that brings unforgettable characters to the big screen.`,
    posterUrl,
  })),
);
const theaters = await Theater.insertMany([
  {
    name: "Premiere Suite",
    description: "Plush reclining seats",
    rows: 7,
    seatsPerRow: 12,
  },
  {
    name: "Grand Hall",
    description: "The classic cinema experience",
    rows: 8,
    seatsPerRow: 14,
  },
  {
    name: "Studio Screen",
    description: "An intimate screening room",
    rows: 6,
    seatsPerRow: 10,
  },
]);
...
for (let i = 0; i < movies.length; i++) {
  const st = await Showtime.create({
    movie: movies[i]._id,
    theater: theaters[i % 3]._id,
    startsAt: new Date(Date.now() + 86400000 * (i + 1) + 3600000 * (17 + i)),
    price: 16 + (i % 2) * 2,
  });
...
console.log(
  "Seeded. Member: member@cinevault.demo / demo1234; Admin: admin@cinevault.demo / demo1234",
);
